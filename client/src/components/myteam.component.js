...
class MyTeam extends Component{
    ...
    componentDidMount(){
        axios.get(`http://localhost:5000/api/users/${this.props.auth.user.id}`)
             ...
             axios.get(`http://localhost:5000/api/users/profileimage/${this.props.auth.user.id}`,{ responseType: 'arraybuffer' },)
             .then(res => {
               const base64 = btoa(
    ...
    render(){
        ...
        if (imagePreviewUrl) {
          $imagePreview = (<img className="my-team-user-img-2" src={imagePreviewUrl} alt="user profile img"/>);
        }
        else{
            $imagePreview = (<img className="my-team-user-img-2" src={this.state.file} alt="user profile img"/>);
        }
        return(
            <div className="my-team-panel">
                <div className="my-team-edit">
                    <button onClick={() => this.setState({showModal1:true, showModal2: false, showModal3: false})}>
                        <PersonAdd/>
                        <br/>
                        Add Player
                    </button>
                    ...
                    <button onClick={() => this.setState({showModal1:false, showModal2: true, showModal3: false})}>
                    ...
                    <button onClick={() => this.setState({showModal1:false, showModal2: false, showModal3: true})}>
                        <Delete/>
                        <br/>
                        Drop Player
                    </button>
                    <Modal show={this.state.showModal3}  onHide={() => this.setState({ showModal3:false})}>
                        ...
                        <Modal.Footer>
                            <Button variant="secondary"  onClick={() => this.setState({ showModal3:false})}>
                                Close
                            </Button>
                            <Button variant="primary">
                                Drop
                ...
                <div className="my-team-user">
                    <div className="my-team-user-img">
                    {$imagePreview}
                    </div>

                    <div className="my-team-username">
                        {this.state.name}
                    </div>
                </div>

                <div className="my-team-starters-wrapper">
                    <div className="my-team-starters-header">
                        Starters
                    </div>
                ...
                <div className="my-team-bench">
                    <div className="my-team-bench-header">
                        Bench
                    </div>

                    <div className="my-team-bench-body">
