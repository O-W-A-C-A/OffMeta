...
class MyTeam extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get(
        `http://localhost:5000/api/leagues/getuserteam/${this.state.leagueID}`,
        {
          params: {
      ...
      .then((res) => {
        ...
        for (let i = 0; i < res.data.size - 1; i++) {
          ...
          if (ModifiedApiName == ModifiedName) {
            this.setState({
              playerImg: res.data.content[i].headshot,
...
*/
  ...
  */
  onSubmitDropPlayer(dropPlayerID) {
    ...
    axios
      .post(
        `http://localhost:5000/api/leagues/dropplayer/${this.state.leagueID}`,
        dropPlayer
      )
      .then((res) => {
        console.log(res.data);
    ...
    return this.state.myTeam.map((player) => (
      <div key={player.playerID} className='drop-player-wrapper'>
        ...
        <div className='drop-btn-wrapper'>
          <button
            className='drop-btn'
            onClick={() => this.onSubmitDropPlayer(player.playerID)}
          >
            DROP
          </button>
        </div>
    ...
    return (
      <div className='my-team-panel'>
        <div className='my-team-edit'>
          ...
          >
            <Modal.Header>
              <Modal.Title style={{ color: 'white' }} className='add-title'>
                Add A Player
              </Modal.Title>
            </Modal.Header>
            <Modal.Body>
            ...
            <Modal.Footer>
              ...
              <Button variant='primary' onClick={this.onSubmitAddedPlayer}>
                Add
              </Button>
            </Modal.Footer>
          </Modal>
        ...
        <div className='my-team-starters-wrapper'>
          <div className='my-team-starters-content'>
            ...
            <div className='my-team-starters-body'>{this.renderMyTeam()}</div>
          </div>
        </div>
      </div>
    );
